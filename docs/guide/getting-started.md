...
::: warning 资源限制
算力使用遵循 [算力使用规则 V2.0](/guide/compute-rules)，要点如下：

- 普通计算任务"排队 + 使用"总额度不超过 **128 卡·时**，超限自动清除
- 正式注册学生享有 **4 卡** 优先额度，保证立即运行
- 战略/聚焦/特殊项目需经学院科研委员会审批
:::
...
|------|------|
| `/models` | **用户主目录（默认工作目录）** |
| `/mnt/host-model` | **约 500 T 大容量共享文件存储（推荐存放项目文件、数据集、模型）** |
| `/models/share` | 共享数据目录 |
