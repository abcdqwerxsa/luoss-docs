...
- **Base URL**: `http://10.1.30.44:8005/v1`
- **Chat Completions**: `http://10.1.30.44:8005/v1/chat/completions`
...
```bash
curl http://10.1.30.44:8005/v1/chat/completions \
...
client = openai.OpenAI(
    base_url="http://10.1.30.44:8005/v1",
...
const openai = new OpenAI({
  baseURL: 'http://10.1.30.44:8005/v1',
...
```javascript
fetch('http://10.1.30.44:8005/v1/chat/completions', {
  method: 'POST',
  headers: {
...
- **Endpoint**: `http://10.1.30.44:8005/v1/responses`

### cURL 示例

```bash
curl http://10.1.30.44:8005/v1/responses \
  -H "Content-Type: application/json" \
  -d '{
    "model": "deepseek_v4",
...
url = "http://10.1.30.44:8005/v1/responses"
payload = {
    "model": "deepseek_v4",
    "input": "写一个关于独角兽的三句话睡前故事。"
}
...
async function createResponse() {
  const response = await fetch('http://10.1.30.44:8005/v1/responses', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
