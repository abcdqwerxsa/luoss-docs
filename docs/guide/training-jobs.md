...
开启后所有 Pod 直接使用宿主机网络（`hostNetwork: true`），并自动设置 `dnsPolicy: ClusterFirstWithHostNet` 以保证集群 DNS 可用。适用于 RDMA/HCCL 等需要主机网络的场景。

| 项目 | 说明 |
|------|------|
| **适用模式** | 单 Task 与自定义多任务模式均支持（任务级开关，作用于所有 Pod） |
| **前端开关** | 任务模式下方的「网络配置」卡片 — 「宿主机网络 (hostNetwork)」 |
| **CLI 参数** | `--host-network` |
| **YAML 字段** | `host_network: true` |

::: warning 注意事项
- 开启后 Pod 共享宿主机网络命名空间，注意端口冲突。
- 一般训练任务无需开启；仅在与主机网络强相关的场景使用。
:::

### 数据盘绑定

创建训练任务时，可选择将一块**空闲数据盘**绑定到任务中。任务运行期间数据盘挂载到容器的 `/mnt/data` 目录，训练脚本可直接读写。

| 项目 | 说明 |
|------|------|
| **前端选择** | 任务模式下方的「数据盘（可选）」卡片 — 下拉选择空闲数据盘 |
| **CLI 参数** | `--data-volume <ID>` |
| **YAML 字段** | `data_volume: <ID>` |
| **挂载路径** | `/mnt/data` |

::: warning 使用限制
- 数据盘为独占式存储，**同一时间只能绑定到一个训练任务**。
- 仅支持**单副本**（replicas=1）任务。
- 任务删除后数据盘自动解绑，恢复为空闲状态，可再次绑定。
:::

::: tip 查看和下载数据盘文件
训练结束后，可通过文件浏览器在线查看和下载数据盘中的文件（如训练输出的模型）。详见 [数据盘](/guide/data-volumes)。
:::
...
| vcjob (单任务模式) | acjob (自定义多任务模式) |
|---|---|
| ● 故障处理模式 (force / grace) | ● 故障处理模式 (force / grace) |
| ● 故障重试次数 | ● 故障重试次数 |
| ● 优雅终止时间 | ● 恢复策略 (多选) |
| | ● Pod 级重调度 |
| | ● 亚健康策略 |
...
为避免训练任务"占卡不干活"（进程没退出但 HBM 显存利用率长期几乎为 0，例如训练已结束忘了停、长时间挂死），平台会**自动回收显存利用率持续处于低位的训练任务**，把空闲占用的 NPU 释放给其他任务。

<FeatureBadge status="beta" />

| 项目 | 说明 |
|------|------|
| 判定口径 | 任务所占各 NPU 中**最忙那张**的 HBM 利用率，过去约 60 分钟内 95% 的时间 ≤10% |
| 预警 | 清理前约 30 分钟会收到 [站内信通知](/guide/notifications)（"训练任务因显存空闲即将被自动清理"） |
| 处理 | 到窗口后自动终止任务（终止原因 `gpu_idle_low_utilization`），**任务记录与数据盘 / 输出保留，可 Restart** |
| 影响范围 | 仅训练任务；开发环境不受影响 |

::: tip 如何避免被回收
确保训练任务**真正在用卡**（前向 / 反向传播会持续占用 HBM，远高于阈值）。如果你的任务**合法地**长时间低显存——例如常驻推理服务、长时间数据下载 / 预处理——请**联系管理员**给任务 Pod 打免清理注解（`tenant.platform/gpu-idle-cleanup=disable`）。该注解仅管理员可通过命令行设置，前端不提供自助入口。
:::

::: warning 判定看的是显存（HBM），不是算力
一个任务 CPU 在跑、但几乎没动显存（如纯数据预处理），仍可能被判空闲。这类任务务必联系管理员加豁免。
:::

详细机制与配置见管理员文档 [显存空闲清理](/admin/gpu-idle-cleanup)。
...
### Q: 我的训练任务为什么被自动终止了（原因 gpu_idle_low_utilization）？
A: 平台会自动回收**显存（HBM）利用率持续低位**（默认过去 60 分钟 ≤10%）的训练任务，终止原因记为 `gpu_idle_low_utilization`。任务记录与数据均保留，可 Restart。如果你的任务本就在用卡（HBM 利用率明显高于 10%）却被误终止，或属于合法低显存负载（推理 / 数据预处理），请联系管理员排查或加免清理注解。详见 [显存空闲自动回收](#显存空闲自动回收) 与 [显存空闲清理（管理员）](/admin/gpu-idle-cleanup)。
